...
if(isset($_POST['submit'])){
	...
	if(!empty($name) &&!empty($intro) && !empty($details) && !empty($fb_link) && !empty($twitter_link) && !empty($instra_link) && !empty($github_link) ){
		...
		$work_insert = $dbcon->query("UPDATE user SET name='$name',intro='$intro',details='$details',facebook='$fb_link',instagram='$twitter_link',linkediin='$instra_link',github='$github_link' WHERE id=1");
		if($work_insert){
	...
	if(!empty($photo)){
		...
		if($_FILES['photo']['size']<5000000){
			...
			if( in_array(strtolower($photo_ext),$accepted_extension_list) ){
				...
				$old_photo_query =$dbcon->query("SELECT photo FROM user WHERE id=$id");
				...
				// upload photo
				$update_photo =$dbcon->query("UPDATE user SET photo='$photo_name' WHERE id=$id");
				if($update_photo){

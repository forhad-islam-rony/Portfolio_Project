<?php
ob_start(); 
require_once "user_auth.php";
$title="Add Education Information";
require_once "header.php";
require_once "db.php";

if(isset($_POST['submit'])){
	$course       = $_POST['course'];
	$institution              = $_POST['institution'];
	$start       = $_POST['start_year'];
	$end       = $_POST['end_year'];
	

	if(!empty($course) &&!empty($institution) && !empty($start) && !empty($end)){
		$work_insert = $dbcon->query("INSERT INTO education(course,institution,start_year,end_year) VALUES('$course','$institution','$start','$end')");
		if($work_insert){
			$_SESSION['education_add_success'] = "Degree add Successfully!";
			header('location: education.php');
			ob_end_flush();
		}
		
	}

}





?>


<!-- Start Page content -->
<div class="card text-dark mb-3" >
	  <div class="card-header bg-success text-center "><h2>Add Degree</h2></div>
	  <div class="card-body">

			<form action="" method="post" >
				<div class="form-group">
					<label for="project_name">Course Name</label>
					<input type="text" class="form-control" name="course">
				</div>

				<div class="form-group">
					<label for="project_catagory">Institution Name</label>
					<input type="text" class="form-control" name="institution">
				</div>

				<div class="form-group">
					<label for="project_catagory">Start Year</label>
					<input type="text" class="form-control" name="start_year">
				</div>
				<div class="form-group">
					<label for="project_catagory">End Year</label>
					<input type="text" class="form-control" name="end_year">
				</div>


				
				
				<div class="form-group">
					<input class="btn btn-block btn-success" type="submit" value="Add" name="submit">
				</div>

			</form>
		</div>
	</div>


<!-- ============ footer content =============== -->
<?php 
    require_once "footer.php";
?>

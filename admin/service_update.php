...
if(isset($_POST['submit'])){
	$icon         = $_POST['icon'];
	$title        = $_POST['title'];
	$some_text    = $_POST['description'];
 	

	if(!empty($title) &&!empty($some_text)){
		$service_update = $dbcon->query("UPDATE services SET icon='$icon',title='$title',description='$some_text' WHERE id=$id");
		if($service_update){
	  	...
	  	<div class="card-body">
						...
						<form action="" method="post" >
							...
							<div class="form-group">
								...
								<textarea name="description" class="form-control" cols="100%" rows="10"><?=$service_data['description']?></textarea>
							</div>
